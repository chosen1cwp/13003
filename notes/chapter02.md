...
- [一、第一节 线性表的定义和基本操作](#一第一节-线性表的定义和基本操作)
- [二、第二节 线性表的顺序存储及实现](#二第二节-线性表的顺序存储及实现)
- [三、第三节 线性表的链式存储及实现](#三第三节-线性表的链式存储及实现)
- [第二章总结](#第二章总结)
- [习题](#习题)
	- [一、单项选择题](#一单项选择题)
	- [二、填空题](#二填空题)
...
3、在单链表L中，已知q所指结点是p所指结点的前驱结点，next是结点的指针域，若在q和p之间插入s所指结点，则执行的操作是（`D`）  
...
9、带头结点的单链表的头指针为head，表为空的判定条件是(`B`)
...
6、设一个非空的静态链表保存在数组中，以-1表示表尾所在结点为表尾结点，则含-1的单元个数最多为（`2`）  
...
1、教材中表2-1给出一个线性表操作示例，请采用顺序存储方式保存线性表，编程使用基本操作验证表中的结论。
...
{	LinearList listtest;
	int i, temp;
	printf("采用顺序表保存线性表\n");
	if ( initList( &listtest )==FALSE ) {
		printf("  初始化失败\n");
		return 0;
	} else printf("  完成初始化\n");
	for ( i=0; i<6; i++ )
		if ( !insertList(&listtest,i,2*i) )	printf("插入错误\n");
	display( &listtest );
	if ( !removeList(&listtest, 3, &temp) ) printf("删除错误\n");
	else {
		printf("删除的3号元素是：%d\n", temp);
		printf("删除后，");
		display( &listtest );
	}
	if ( !setValue(&listtest, 3, -10) )	printf("修改值出错\n");
	else {
		printf("修改3号元素的值，");
		display( &listtest );
	}
	if ( !getValue(&listtest, 3, &temp) ) printf("获取值出错\n");
	else printf("获取3号元素的值：%d\n", temp);
	printf("查找 10 的结果: %d\n", find(&listtest, 10));
	printf("查找  9 的结果: %d\n", find(&listtest, 9));
	return 0;
}
...
1、简述顺序表和链表的插入效率差异及原因

	顺序表：需移动元素，时间复杂度为 O(n)。
	链表：仅修改指针，时间复杂度为 O(1)。
	原因：顺序表需保持物理连续性，链表通过指针灵活链接。

2、简述顺序表插入和删除操作的实现过程及时间复杂度分析

	插入时需移动后续元素，时间复杂度 O (n)
	删除时同样需移动元素，时间复杂度 O (n)。

3、简述对比线性表顺序存储和链式存储的优缺点

	顺序存储可随机访问但插入删除效率低
	链式存储动态灵活但需额外指针空间

4、简述单链表中头结点的作用，以及在带头结点的单链表中插入和删除结点的操作步骤

	头结点简化边界条件处理。
	插入时修改指针指向，删除时调整前驱指针并释放结点。

5、简述什么是静态链表？它有什么特点？

	静态链表使用数组模拟链表，结合顺序表和链表的优点，插入删除无需移动元素但需预先分配空间。
